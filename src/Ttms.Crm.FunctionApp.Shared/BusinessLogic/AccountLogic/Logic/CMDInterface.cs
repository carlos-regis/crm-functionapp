...
using Ttms.Crm.FunctionApp.Shared.Helpers;
...
namespace Ttms.Crm.FunctionApp.Shared.BusinessLogic.AccountLogic.Logic
{
    public class CMDInterface
    {
        ...
        private readonly Account accountToUpdate;
        private readonly Entity postImage;
        private readonly CountryRepos countryRepos;
        private readonly ConfigurationParameterRepos configurationParameterRepos;
        private readonly BoschSiteRepos boschSiteRepos;
        private readonly CustomerMasterDataExchangeRepos customerMasterDataExchageRepos;

        public CMDInterface(ILogger _logger, Entity _entityTarget, Account _accountToUpdate,
        ...
        {
            logger.LogInformation("CMDInterface_Logic --> Started");
            ...
            {
                logger.LogInformation("--- RequestCMD ---");

                RequestCMD requestCMD = new RequestCMD(logger, brand, countryISO, erpNumber, retrievedConfigurationParameters);
                ...
                if (getCustomerUpdateReference != null)
                {
                    ...
                    if (getCustomerUpdateReference.GetCustomerInformationResponse.Status != null)
                    {
                        if (checkIfNullOrEmpty(getCustomerUpdateReference.GetCustomerInformationResponse.Status.Blocked, Account.Fields.rbtt_IsBlocked))
                            accountToUpdate.Attributes[Account.Fields.rbtt_IsBlocked] = getCustomerUpdateReference.GetCustomerInformationResponse.Status.Blocked != "No";
                    }
                    else
                ...
                {
                    logger.LogInformation("GetCMDEByAccountId --> Not Found");
                }
            }

            logger.LogInformation("CMDInterface_Logic --> Ended");

            return accountToUpdate;
        }

        private string getErpNumber()
        {
            if (postImage.Attributes.Contains(Account.Fields.rbtt_ERPCustomerNumber))
            {
                return postImage.Attributes.Contains(Account.Fields.rbtt_ERPShiptoNumber) ?
                    postImage.Attributes[Account.Fields.rbtt_ERPShiptoNumber].ToString() :
                    postImage.Attributes[Account.Fields.rbtt_ERPCustomerNumber].ToString();
            }
